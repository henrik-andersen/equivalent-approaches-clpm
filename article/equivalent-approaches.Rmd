...
- emphasize that while it is widely known that observation-level models can be re-expressed as residual-level models [see @Jongerling2011; @Hamaker2005; @Hsiao2014, p. 86f.; @Ou2016], they result in 'constrained' versions of the observation-level counterparts, 
...
\end{align}
So, we can constrain the factor loadings of $\eta_{1_{i}} \rightarrow y_{i0}$ and $\eta_{2_{i}} \rightarrow y_{i0}$ to these fixed values, saving two degrees of freedom. For the constrained DPM, we drop the latent slope and are left with just $y_{i0} = (1 - \rho)^{-1}\eta_{i} + \varepsilon_{i0}$. A four-wave version of the constrained observation-level model with individual effects is shown in Figure \ref{fig:ar_con}. 
...
\end{split}
\end{align}
The RI-CLPM is essentially the same, but with no latent slope,
\begin{align}
\begin{split}
...
\end{split}
\end{align}
where $\E(\alpha_{1_{i}})$ can be thought of as the individual's intercept and $\E(\alpha_{2_{i}})$ can be thought of as their slope [@Ou2016; @Hamaker2005]. For the ALT and DPM, the same is not true, as 
\begin{align}
\begin{split}
...
\end{split}
\end{align}
means the expected value of the dependent variable is more complicated than $\E(\eta_{1_{i}})$ and $t\E(\eta_{2_{i}})$ and these cannot be interpreted as the intercept and slope factors. So, while the residual-level models allow for an easier interpretation of the latent individual effects, attempting to model the mean structure solely by the latent individual effects, as is common practice in LCMs and ALT models, is problematic because it ignores occasion effects [@Zyphur2019b]. 

Note that we can take the residual-level models and insert the equation for the residuals into the equation for the observations. For the LCM-SR in what we can call *reduced-form*, we have 
\begin{align}
\begin{split}
y_{it} & = \alpha_{1_{i}} + t\alpha_{2_{i}} + \rho \varepsilon_{it-1} + \nu_{it} \\
...
\begin{align}
y_{i0} & = (1 - \rho)^{-1}\eta_{1_{i}} - \rho(1 - \rho)^{-2}\eta_{2_{i}} + \nu_{i0}
\end{align}
since without $\varepsilon_{it-1}$, we just have $\varepsilon_{i0} = \nu_{i0}$. This is exactly the equation for the first observation in the *constrained* ALT from Equation \eqref{eq:alt-cons-y0}. The same goes for the RI-CLPM, which is equivalent to a constrained type of DPM. To see how, take the RI-CLPM in reduced-form
\begin{align}
\begin{split}
y_{it} & = \alpha_{i} + \rho \varepsilon_{it-1} + \nu_{it}, \ t = 1, \ldots, T \\
 & = \alpha_{i} + \rho (y_{it-1} - \alpha_{i}) + \nu_{it} \\
 & = (1 - \rho)\alpha_{i} + \rho y_{it-1} + \nu_{it}
\end{split}
\end{align}
and define $\eta_{i} = (1 - \rho)\alpha_{i}$ for
\begin{align}
y_{it} & = \eta_{i} + \rho y_{it-1} + \nu_{it}, \ t = 1, \ldots, T
\end{align}
which is the DPM for $t = 1, \ldots, T$. For $y_{i0}$, we have 
\begin{align}
\begin{split}
y_{i0} & = \alpha_{i} + \varepsilon_{i0} \\
 & = (1 - \rho)^{-1}\eta_{i} + \nu_{i0}
\end{split}
\end{align}
which is the equation for $y_{i0}$ in the constrained version of the DPM. The equivalence holds for cross-lagged or 'two-sided' models as well. To see how, take the RI-CLPM, 
\begin{align}
\begin{split}
...
\delta_{it} & = \varphi \delta_{it-1} + \gamma \varepsilon_{it-1} + \upsilon_{it}, \ t = 1, \ldots, T
\end{split}
\end{align}
...
x_{it} & = \varphi x_{it-1} + \theta y_{it-1} + (1 - \varphi)\alpha^{(x)}_{i} - \gamma \alpha^{(y)}_{i} + \upsilon_{it}, \ t = 1, \ldots, T.
\end{split}
\end{align}
...
\begin{align}
\begin{split}
y_{i0} & = \varpi^{(y)}\eta^{(y)}_{i} + \omega^{(x)}\eta^{(x)}_{i} + \nu_{i0} \\
x_{i0} & = \varpi^{(x)}\eta^{(x)}_{i} + \omega^{(y)}\eta^{(y)}_{i} + \upsilon_{i0}, \\
y_{it} & = \rho y_{it-1} + \beta x_{it-1} + \eta^{(y)}_{i} + \nu_{it} \\
x_{it} & = \varphi x_{it-1} + \theta y_{it-1} + \eta^{(x)}_{i} + \upsilon_{it}, \ t = 1, \ldots, T
\end{split}
\end{align}
where $\eta^{(y)} = (1 - \rho)\alpha^{(y)}_{i} - \beta \alpha^{(x)}_{i}$ and $\eta^{(x)}_{i} = (1 - \varphi)\alpha^{(x)}_{i} - \theta \alpha^{(y)}_{i}$ and 
...
The previous section showed that the residual-level models are equivalent to constrained observation-level models, and that these differ from the predetermined observation-level models *solely* in their treatment of the initial observations. 

This has two important implications. First, it means the residual models offer no benefit in terms of the estimated within-unit coefficients. Afterall, when we write an observation-level model as a residual-level model, like in Equations \eqref{eq:alt-y0} and \eqref{eq:alt-y0-2}, the coefficient(s) did not change, we just constrained the initial factor loading(s) and redefined the individual effects. As we will see below, if the assumptions concerning the initial conditions hold, then the residual and constrained models are essentially identical to the predetermined model in terms of estimated AR and CL effects; the predetermined model just uses more degrees of freedom in the process. Secondly, those assumption placed on the initial conditions are potentially very restrictive. In this section, we will discuss these assumptions in detail and look at situation in which they are likely not met. 

For the sake of simplicity, let us focus on the DPM and RI-CLPM outlined in Equations \eqref{eq:dpm} and \eqref{eq:riclpm}. Most of what can be shown for the models with individual intercepts can be shown as well for those with individual intercepts and slopes, but it becomes more complicated and less intuitive. The empirical examples that follow will show that the results hold for the ALT and LCM-SR, as well. 

From any regression model, we know that the model covariates must be independent of the error. If they are not, then estimated coefficients can be biased and inconsistent [@Wooldridge2002]. Let us examine the initial equation in the constrained model (which is, as we have established, equivalent to the residual-level model) since it is the only thing differentiating the constrained from the predetermined model. For $y_{i0}$, we have $y_{i0} = (1 - \rho)^{-1}\eta_{i} + \nu_{i0}$. Here, the covariate is $\eta_{i}$, even though it is unobserved, and we have to establish $\Cov(\eta_{i},\nu_{i0}) = 0$ for consistency (the stronger assumption, $\E(\nu_{i0} | \eta_{i}) = 0$ is needed for unbiasedness, but for most practical purposes we will be interested primarily in the covariance). 

So, we can write 
\begin{align}
\begin{split}
y_{i0} & = (1 - \rho)^{-1}\eta_{i} + \nu_{i0} \\
\nu_{i0} & = y_{i0} - (1 - \rho)^{-1}\eta_{i}
\end{split}
\end{align}
and 
\begin{align}
\begin{split}
\Cov(\nu_{i0},\eta_{i}) & = \E[(y_{i0} - (1 - \rho)^{-1}\eta_{i})\eta_{i}] - \E(y_{i0} - (1 - \rho)^{-1}\eta_{i})\E(\eta_{i}) \\
...
\end{split}
\end{align}
so that for this equation to equal zero, the covariance between the initial observation and the individual effects *must* equal $(1 - \rho)^{-1}\Var(\eta_{i})$, which is the assumption of the constrained and residual-level models. We arrive at this covariance by assuming $\rho_{t} = \rho$ and $|\rho| < 1$ and continually substituting in the equation for the previous variable into the current one. But it is crucial to note that this is an *assumption*. @Usami2019 address this when they write: "Note that in the RI-CLPM, the initial within-person centered scores [...] are not correlated with the stable trait factors [...] by definition. The reason for this is that the model is based on decomposing the observed variance into between-person/trait-like variance and within-person/state-like variance. There is no reason to assume that an individual's temporal deviation at the first measurement from the person's trait score is dependent on that trait score (especially not when the measurements started at a relatively arbitrary moment in time)" [-@Usami2019, p. 640].^[The notation used in @Usami2019 was removed to avoid confusion.]

The problem is that the covariance between the initial observation and the individual effects is *unobserved*. So how can we know whether it is appropriate to fix the covariance to $(1 - \rho)^{-1}\Var(\eta_{i})$? Fortunately, the assumptions surrounding the initial conditions impact the model-implied moments of the residual and constrained models, so that looking at the empirically observed moments can help diagnose whether these have been violated.  

Namely, residual and constrained models assume the process $y_{it}$ is *covariance stationary* and at *equilibrium*. Only then will $\Cov(y_{i0},\eta_{i}) = (1 - \rho)^{-1}\Var(\eta_{i})$. Covariance stationary means that the expected value of the dependent variable, $\E(y_{it})$, is constant over time, the variance, $\Var(y_{it})$, is constant over time (assuming the variance of the idiosyncratic error is constant over time, as well) and autocovariance, $\Cov(y_{it},y_{it+h})$, depends on the lag, $h$, but not on the specific point in time, $t$ [@Pickup2015; @Hamaker2009]. Equilibrium means that the process has been going on long enough to have reached these conditions [@Greenberg1982].^[As discussed in @Hamaker2009, models with individual slopes are sometimes referred to as having a 'deterministic trend'. Because of this trend, the means of the observed variable are always increasing or decreasing. Such processes are referred to as 'trend stationary' if, once the trend has been removed [i.e., by differencing or detrending, @Bruederl2015; @Hamaker2009], the means do not change over time, though the variance may still be time-varying.]

To see why this is the case, let us look at the model-implied moments of the constrained model. We will focus on the constrained model rather than the residual-level one to emphasize the fact that stationarity and equilibrium boil down to the constraint placed on the initial factor loading and that freeing this factor loading gives us back the more flexible predetermined model.^[In fact, the model-implied moments are much easier to derive using the residual-level formulation.] In the Appendix, the model-implied moments of both the constrained and residual-level models will be shown in more detail. 

Compared to the residual-level formulation, it is not as obvious in the constrained model why fixing the initial factor loading to $(1 - \rho)^{-1}$ and the subsequent observations to 1.0 should imply constant means, variances and autocovariances. For the sake of exposition, let us assume there are no occasion effects, i.e., the mean structure can be modeled entirely with just the latent intercept. As noted earlier, this is often an unrealistic assumption, and it is probably wiser to avoid placing restrictions on the mean structure by either running the model on centered data, or allowing an intercept per point in time to be estimated freely [@Zyphur2019b; @Hamaker2015]. Assuming there are no occasion effects, taking the expectation of $y_{it}$ gives us
\begin{align}
\begin{split}
\E(y_{it}) = & \E(\eta_{i} + \rho y_{it-1} + \nu_{it}) \\
 = & \E(\eta_{i}) + \rho \E(\eta_{i} + \rho y_{it-2} + \nu_{it-1}) \\
 = & \E(\eta_{i}) + \rho \E(\eta_{i}) + \rho^{2}\E(\eta_{i} + \rho y_{it-3} + \nu_{it-2}) \\
 = & \ldots \\
 = & \sum_{j=0}^{\infty}\rho^{j}\E(\eta_{i}) \\
 = & (1 - \rho)^{-1}\E(\eta_{i}) 
\end{split}
\end{align}
since $\E(\nu_{it}) = 0, \ \forall \ t$. Again, as long as $\rho_{t} = \rho$ and $|\rho| < 1$, then this should hold for any $t$. 

By proceeding in a similar fashion (see the Appendix for more detail), we get the other model-implied moments: 
\begin{align}
\begin{split}
\Var(y_{it}) & = (1 - \rho)^{-2}\Var(\eta_{i}) - (\rho^{2} - 1)^{-1}\Var(\nu_{it}) \\
\Cov(y_{it},y_{it-1}) & = (1 - \rho)^{-2}\Var(\eta_{i}) - \rho(\rho^{2} - 1)^{-1}\Var(\nu_{it})
\end{split}
\end{align}
where we can find the model-implied covariance between observations separated by more than one point in time by replacing the $\rho$ in front of the brackets in the second term with $\rho^{j}$, where $j$ is the distance in time between the two. 

As noted above, the covariance between the observed variable and the individual intercept is also assumed constant at 
\begin{align}
\begin{split}
\Cov(y_{it},\eta_{i}) & = (1 - \rho)^{-1}\Var(\eta_{i}).
\end{split}
\end{align}
This can be shown by continual substitution of the previous equations into the current one, or even more easily by looking at Figure \ref{fig:const-cov}, where by path tracing, we can see $\Cov(y_{3},\eta)$, for example, is $\Var(\eta) + \rho \Var(\eta) + \rho^{2}\Var(\eta) + \rho^{3}(1  - \rho)^{-1}\Var(\eta) = (1 - \rho)^{-1}\Var(\eta)$.^[Consider an example where $\rho = 0.5$ and $\Var(\eta) = 2$, then $2 + 0.5 \cdot 2 + 0.5^{2} \cdot 2 + 0.5^{3} (1 - 0.5)^{-1} \cdot 2 = (1 - 0.5)^{-1} \cdot 2 = 4$.]
...
%\path (eta) edge [cor, bend left] node {} (eta2);
\end{tikzpicture}
\caption{Constant covariance between $y_{it}$ and $\eta_{i}$ in the constrained model} \label{fig:const-cov}
\end{figure}

So, we can look at the empirical means, variances and covariances of the observed variable over time and diagnose whether we should fix the initial covariance, $\Cov(y_{i0},\eta_{i})$ to $(1 - \rho)^{-1}\Var(\eta_{i})$, or whether it would be preferable to switch to a predetermined model and allow this to be estimated freely. The following section will show using a brief simulation how inspecting the empirical means, variances and covariances can help determine whether a residual or constrained model is appropriate, or not. 

## A brief simulated example

To demonstrate what has been shown up until now, we can look at a brief example using simulated data based on the equation $y_{it} = \eta_{i} + \rho y_{it-1} + \nu_{it}$, with $n = 10,000$, $t = 15$, $\rho = 0.5$, $y_{0} \sim N(0, 1)$, $\eta \sim N(5, 1)$ and $\nu_{t} \sim N(0, 0.25)$. These values were chosen mostly arbitrarily, though the fact that $\E(\nu_{t}) = 0$ means there are no occasion effects. Code for this simulated example, as well as the empirical examples to follow can be found at https://github.com/henrik-andersen/equivalent-approaches/tree/main/r-files. 
...
\Var(y_{it}) & = (1 - \rho)^{-2}\Var(\eta_{i}) - \frac{\Var(\nu_{it})}{\rho^{2} - 1} = (1 - 0.5)^{-2} \cdot 1 - \frac{0.25}{0.5^{2} - 1} = 4.33, \\
\Cov(y_{it},y_{it-1}) & = (1 - \rho)^{-2}\Var(\eta_{i}) - \rho\frac{\Var(\nu_{it})}{\rho^{2} - 1} = (1 - 0.5)^{-2} \cdot 1 - 0.5 \frac{0.25}{0.5^{2} - 1} = 4.17, \\
\Cov(y_{it},\eta_{i}) & = (1 - \rho)^{-1}\Var(\eta_{i}) = (1 - 0.5)^{-1} \cdot 1 = 2.
\end{align}

```{r echo=FALSE, fig.width=10, fig.height=5, fig.cap="\\label{fig:sim-ex}Simulated example, subsample of five randomly chosen cases"}
...
Figure \ref{fig:sim-ex} shows five randomly drawn cases from the simulated data. As can be seen, the process begins with $y_{i0} \sim N(0, 1)$ and, because the process is stationary, quickly moves towards its equilibrium state. This sets in after around the 5th or 6th timepoint. After that, the process is centered around $\E(y_{it}) = 10$, with a constant variance. Each individual has their own expected value, based on their own value of $\eta_{i}$. For example, case number 1039, (circle) has a corresponding individual effect of $\eta_{i = 1039} = `r round(eta[1039], 3)`$. Thus, their equilibrium state is centered over time around $(1 - \rho)^{-1}\eta_{1039} = (1 - 0.5)^{-1}`r round(eta[1039], 3)` = `r round((1 - 0.5)^-1*eta[1039], 3)`$.  

Once the process has reached equilibirum, the model-implied moments discussed above are appropriate. Importantly, once the process is at equilibrium, $\Cov(y_{it},\eta_{i}) = (1 - \rho)^{-1}\Var(\eta_{i})$. However, this equilibrium state is not reached immediately, but rather requires a 'spin-up' phase. I.e., in the initial phase, up until about $t = 6$ or so, the covariance, $\Cov(y_{it},\eta_{i})$, does not equal $(1 - \rho)^{-1}\Var(\eta_{i})$. For example, at $t = 2$, the covariance is only around `r round(cov(df2$eta, df$y2), 2) ` $\ne (1 - 0.5)^{-1}\cdot 1 = 2$. This means $\Cov(\nu_{it},\eta_{i}) \ne 0$ and thus the coefficient of interest, $\rho$, should we attempt to model the process using the constrained or residual-level model, will be biased. Again, all of this boils down to the treatment of the initial observation, and the constraint of fixing the initial factor loading to $(1 - \rho)^{1}$ so that $\E(y_{it})$, $\Var(y_{it})$, $\Cov(y_{it},y_{it+h})$ and $\Cov(y_{it},\eta_{i})$ are all constant.

@Usami2019 even comment on this, saying the "treatment of the initial observation is an important theoretical and practical problem for some cross-lagged models like the ALT model that includes common factors but do not separate stable between-person differences from within-person fluctuations over time" [@Usami2019, p. 641]. This seems to suggest that the RI-CLPM is spared from having to deal with this issue, but as we have seen in the above example, it is absolutely not; it just ignores it. For the residual-level models to be appropriate, we must be sure that the process we are modeling is indeed stationary and at equilibrium. If it is not, the residual-level models will return biased results for the AR effects as well as potentially those of the other model covariates. 

Even though the covariance between the observed variable and the individual intercept is arguably the most important for the unbiasedness of the estimated coefficients, it is unobserved, so it is impossible to assess the assumption surrounding this covariance in applied settings. But as we have seen, the way we treat the initial observation affects the model-implied moments for which we have observed couterparts, as well. This means that the means, variances and autocovariances of the observed variable can help us diagnose situations in which it is not appropriate to assume stationarity and equilibrium. 

In applied cases with large sample sizes, it may not be feasible to look at the trajectories of each of the observed units, and plotting a subset of them, like in Figure \ref{fig:sim-ex}, may not always reveal unambiguous information. So, it may be more helpful to simply plot the means, variances and autocovariances of the entire sample. Figure \ref{fig:sim-means-vars} does this, and summarizes what was shown in Figure \ref{fig:sim-ex} for the entire simulated sample. The dots represent the observed means and variances, while the horizontal lines represent the model-implied means and variances of the residual- and constrained models, which are constants. 

```{r echo=FALSE, fig.cap="\\label{fig:sim-means-vars}Difference between observed and residual-level model-implied means, variances and covariances (simulated example, dots are observed, lines are model-implied)", fig.subcap=c("\\label{fig:sim-means}Observed and model-implied means", "\\label{fig:sim-vars}Observed and model-implied variances", "\\label{fig:sim-covs}Observed and model-implied autocovariances", "\\label{fig:sim-cov-eta}Observed and model-implied covariances with individual intercepts (only observed in simulations)"), fig.ncol=2, fig.align="center", error=FALSE, message=FALSE, warning=FALSE, out.width="45%"}
...
\end{split}
\end{align}
[@Hayduk2003], where $\hat{\rho}$ would be the estimated autoregressive coefficient. Obviously, if we fix the value of $\Cov(\eta_{i},y_{it-1})$ to $(1 - \rho)^{-1}\Var(\eta_{i})$ and our assumption about stationarity and/or equilibrium does not hold, then we will be subtracting off too much or too little (in the simulation above, it is too much since $\Cov(\eta_{i},y_{it-1}) < (1 - \rho)^{-1}\Var(\eta_{i})$ in the first few realizations) from the autocovariance and the estimated coefficient for $\rho$ will be biased. If we consider the fixed effects regression in SEM, which works by allowing the model covariates to freely covary with the latent individual effects, then we can go so far as to say that the residual-level models *are not fixed effects models* in the sense that they control for all time-invariant unobserved heterogeneity *unless the data is stationary and at equilibrium*. We can extrapolate from here what an happen to the effects of other covariates. Essentially, whenever the covariance between the lagged dependent variable and the individual effects is wrong, the the effects of the other covariates will likely be wrong. And in two-sided residual-level models, all this applies equally to the other time-varying variable. If it is not at equilibrium, then we can expect the effects to be biased in the other direction, as well. 

# Example: Use of cigarettes and alcohol in adolescents and young adults 
...
2. that the implicit assumptions placed on the initial conditions can have serious consequences for the estimated coefficients. 

While smoking amongst adolescents is not necessarily a strictly psychological issue, it and other related behaviours are often looked at in combination with psychological and sociological constructs. @Keijsers2016, for example, used the RI-CLPM to examine the reciprocal relations between parental monitoring and adolescent problem behaviours, which can encompass substance abuse and smoking, risky sexual behaviour, shoplifting, etc. Other studies, like that of @Barnett2013; @Nkansah-Amankra2016; @Bierhoff2019; @Tarter2006 look at the effects of future time perspective, depression, anxiety, ADHD, etc. on substance use and problem behaviours in adolescents. @Lloyd-Richardson2002 looked at the effect of psychological and social influences at various life-stages on substance abuse and @Nelson2014 compared the trajectories of alcohol, marijuana nd smoking behaviour of 12- to 24-year-olds, based on demographic characteristics like sex and race. Another sociological example includes @Schuler2019, who looked at perceived substance abuse amongst peers and family members and its influence on adolescents' own substance use and abuse. 

I use 14 (but focus on six) waves of the National Longitudinal Survey of Youth 1997 (NLYS97) study, spanning the years 1997 to 2011 [@NLYS97]. These are the years for which there is yearly data on both smoking and drinking (after 2011, the variables are measured every two years). The NLYS study (especially the previous version started in 1979) has been used in some prominent papers on panel SEM, see, for example, @Bollen2010; @Allison2009, and it is freely available to download. The NLYS97 study encompasses a probability sample of 8984 youths between the ages of 12 and 17 in 1997 in the United States. 
...
However, young children, except in rare cases, do not smoke cigarettes. An individual's pattern of smoking typically only emerges in adolescence and, for a given youth, this pattern likely changes over time. The development of smoking behaviour therefore likely follows a similar trajectory to the one shown in Figure \ref{fig:sim-ex}. At ages 12-14, the age at which the majority of the NLYS97 sample was surveyed for the first time, we would not expect much variance. Most youths in this age group do not smoke much, if at all. The overall average number of cigarettes smoked per occasion will be low at the first timepoint, as well. As these youths become older, some of their smoking behaviour will change; some start going to parties where they might smoke many cigarettes on a given occasion. Therefore, the variance as well as the average number of cigarettes smoked per timepoint will both increase over time. We would not necessarily expect these to increase indefinitely, but rather settle into a 'normal' pattern of behaviour. At this point, the process will have reached equilibrium, where it could stay for some time, though likely not forever.
...
To demonstrate what has been discussed up until now, I run four 'scenarios': 1) a simple autoregressive one-sided model with individual intercepts, 2) an extended autoregressive one-sided model with individual intercepts and a time-varying and time-invariant covariate, 3) an extended one-sided autoregressive model with individual intercepts and slopes and 4) a full cross-lagged model with individual intercepts. For each of these four types of models, I run a residual-level model, a constrained model and a predetermined model. I will show that in each case, the residual and constrained model are equivalent. The predetermined model differs from the other two in the way it treats the initial observations, and so it will differ, sometimes substantially, from the other two. Within each of the four scenarios, the residual and constrained models are nested within the predetermined one, so a chi-square difference test will be carried out to help put the differences into perspective. Throughout, I use full-information maximum likelihood (FIML) to account for missing values and panel attrition. 
...
In lav_model_vcov(lavmodel = lavmodel, lavsamplestats = lavsamplestats,  :
  lavaan WARNING:
    ...
    (= 1.956191e-14) is close to zero. This may be a symptom that the
...
\end{split}
\end{align}
where we derived the factor loadings for the initial observation $cig_{i97}$ above. For the predetermined model, we have 
\begin{align}
...
Table \ref{tab:cig-ex-4} shows the results of th full cross-lagged models with individual intercepts and no time-invariant covariates. They examine the 'within-person' relations between cigarette smoking and alcohol use in adolescents and young adults. The residual-level model is a true RI-CLPM, the constrained and predetermined models are akin to a 'two-sided' DPM, though @Allison2017 only ever really discuss the predetermined variant. 

Here, again, we see the autoregressive coefficient for smoking being estimated much higher in the residual and constrained models than in the predetermined one. The lagged effect of drinking on smoking is comparable between the models, but the effect is slightly higher in the predetermined one. The autoregressive effect of drinking is also comparable between the models, while the lagged effect of smoking on drinking is only significant in the residual and constrained formulations. 

The fit is fairly drastically better in the predetermined model, and the chi-square difference is again significant, as can be seen in the output below. The models differ in four degrees of freedom. Those are the freely estimated covariances in the predetermined model between the initial observations $cig_{i97}$ and $drink_{i97}$ and the two latent individual intercept variables, $\eta_{i}^{(p,y)}$ and $\eta_{i}^{(p,y)}$. 

```{r echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
m4a.fit <- readRDS("../r-files/m4a_fit.rda")
m4b.fit <- readRDS("../r-files/m4b_fit.rda")
m4c.fit <- readRDS("../r-files/m4c_fit.rda")

# --- Likelihood ratio test
anova(m4a.fit, m4b.fit, m4c.fit)
```

These examples all underline the importance of the assumptions surrounding the initial conditions. While it is likely not wise to try to compare results between the tables (we do not know which is the 'best' modeling approach, e.g., with or without a latent slope, in a one- or two-sided formulation), but the residual and constrained models are nested within the predetermined one within a given table, so those comparisons are valid. In each case, the predetermined model fit significantly better than the residual and constrained ones. This should come as no surprise given Figure \ref{fig:cig}: the data does not support the assumptions inherent to those models. 

To emphasize one of the points made above, that the residual-level models do not offer an advantage in terms of the 'within-person' effects, let us compare the residual, constrained and predetermined models in a final example, using the years between 2005 and 2010. From Figure \ref{fig:cig}, we see that at that point, the data essentially embodies the assumptions of the residual and constrained models. 
...
\Cov(y_{it},y_{it-1}) & = (1 - \rho)^{-2}\Var(\eta_{i}) + \rho\Var(\varepsilon_{it}) \\
 & = (1 - \rho)^{-2}\Var(\eta_{i}) - \rho(\rho^{2} - 1)^{-1}\Var(\nu_{it}).
\end{split}
\end{align}
